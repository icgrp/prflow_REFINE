...
#pragma HLS INTERFACE axis register port=Output_2
  ...
  #pragma HLS array_partition variable=label_local cyclic factor=2
...
#pragma HLS PIPELINE II=1
  ...
  TRAINING_INST: for( int training_id = 0; training_id < NUM_TRAINING; training_id ++ ){
    ...
    READ_TRAINING_DATA_top: for (int i = 0; i < NUM_FEATURES / D_VECTOR_SIZE / 8; i ++ ) // D_VECTOR_SIZE = 4
    {
#pragma HLS PIPELINE II=2
    ...
    READ_TRAINING_DATA_bot: for (int i = 0; i < NUM_FEATURES / D_VECTOR_SIZE / 8; i ++ ){
#pragma HLS PIPELINE II=2
      in_tmp = Input_1.read(); 
      for(int j=0; j<4; j++){
        out_tmp(31, 0) = in_tmp(64*j+31, 64*j+0 );
