...
#define NUM1 1
void update_knn_i1(hls::stream<ap_uint<256> > & Input_1,
...
static int index = 0;
  ...
  if (index == 0)
  {
    ...
    STORE_LOCAL: for(int i = 0; i < NUM_TRAINING / NUM_OPS / 2; i++)
    {
#pragma HLS pipeline
      in_tmp = Input_1.read();
      ...
      in_tmp = Input_1.read();
      training_set[2*i+1](255, 224) =in_tmp(31,    0);
      training_set[2*i+1](223, 192) =in_tmp(63,   32);
      training_set[2*i+1](191, 160) =in_tmp(95,   64);
      training_set[2*i+1](159, 128) =in_tmp(127,  96);
      training_set[2*i+1](127,  96) =in_tmp(159, 128);
      training_set[2*i+1](95,   64) =in_tmp(191, 160);
      training_set[2*i+1](63,   32) =in_tmp(223, 192);
      training_set[2*i+1](31,    0) =in_tmp(255, 224);
    }

    //Transit the training sets for other pages
    TRANSFER_LOOP: for(int i = 0; i < NUM_TRAINING / NUM_OPS * (NUM_OPS - NUM1) / 2; i++)
    {
#pragma HLS pipeline
      in_tmp = Input_1.read();
...
#pragma HLS array_partition variable=label_list complete dim=0
  ...
  {
#pragma HLS unroll
    min_distance_list[i] = IMAGE_SIZE;
    label_list[i] = 0;
  }
