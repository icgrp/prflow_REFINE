#include "../host/typedefs.h"
#include "../host/finn-hlslib/bnn-library.h"
#include "../host/finn-hlslib/activations.hpp"
#include "../host/finn-hlslib/maxpool.h"
#include "../host/finn-hlslib/mvau.hpp"
#include "../host/finn-hlslib/streamtools.h"
#include "../host/finn-hlslib/weights.hpp"

// ------------------------------------------------------------------------


#define AP_INT_MAX_W_str_dwc_5 72



// defines for network parameters
#define InWidth_str_dwc_5 8 
#define OutWidth_str_dwc_5 72 
#define NumInWords_str_dwc_5 7200 
#define numReps_str_dwc_5 1

void str_dwc_5(hls::stream<ap_uint<8> > &in0, hls::stream<ap_uint<72> > &out)
{
#pragma HLS INTERFACE axis register port=in0
#pragma HLS INTERFACE axis register port=out
StreamingDataWidthConverter_Batch<InWidth_str_dwc_5, OutWidth_str_dwc_5, NumInWords_str_dwc_5>(in0, out, numReps_str_dwc_5);
}

// ------------------------------------------------------------------------


#define AP_INT_MAX_W_mva_3 72


#include "mva_3_thresh.h"

// defines for network parameters
#define MW1_mva_3 576
#define MH1_mva_3 64
#define SIMD1_mva_3 72
#define PE1_mva_3 8
#define WMEM1_mva_3 64
#define TMEM1_mva_3 8
#define numReps_mva_3 100

void mva_3(hls::stream<ap_uint<72>> &in0,
                    hls::stream<ap_uint<8>> &out
                    )
{
#pragma HLS INTERFACE axis register port=in0
#pragma HLS INTERFACE axis register port=out
#include "mva_3_params.h"
#pragma HLS ARRAY_PARTITION variable=mva_3_weights.m_weights complete dim=1
#pragma HLS ARRAY_PARTITION variable=mva_3_threshs.m_thresholds complete dim=1
#pragma HLS ARRAY_PARTITION variable=mva_3_threshs.m_thresholds complete dim=3
Matrix_Vector_Activate_Batch<MW1_mva_3, MH1_mva_3, SIMD1_mva_3, PE1_mva_3, 1, Recast<XnorMul>, Slice<ap_uint<1>>, Identity>
                (in0, out, mva_3_weights, mva_3_threshs, numReps_mva_3, ap_resource_lut());
}

// ------------------------------------------------------------------------


#define AP_INT_MAX_W_str_dwc_6 64



// defines for network parameters
#define InWidth_str_dwc_6 8 
#define OutWidth_str_dwc_6 64 
#define NumInWords_str_dwc_6 800 
#define numReps_str_dwc_6 1

void str_dwc_6(hls::stream<ap_uint<8> > &in0, hls::stream<ap_uint<64> > &out)
{
#pragma HLS INTERFACE axis register port=in0
#pragma HLS INTERFACE axis register port=out
StreamingDataWidthConverter_Batch<InWidth_str_dwc_6, OutWidth_str_dwc_6, NumInWords_str_dwc_6>(in0, out, numReps_str_dwc_6);
}

// ------------------------------------------------------------------------


#define AP_INT_MAX_W_str_mp_batch_1 64



// defines for network parameters
#define ImgDim_str_mp_batch_1 10
#define PoolDim_str_mp_batch_1 2
#define NumChannels_str_mp_batch_1 64
#define numReps_str_mp_batch_1 1

void str_mp_batch_1(hls::stream<ap_uint<64>> &in0, 
                       hls::stream<ap_uint<64>> &out)
{
#pragma HLS INTERFACE axis register port=in0
#pragma HLS INTERFACE axis register port=out
StreamingMaxPool<ImgDim_str_mp_batch_1, PoolDim_str_mp_batch_1, NumChannels_str_mp_batch_1>(in0, out);
}

// ------------------------------------------------------------------------

void layer_3_1 (
        hls::stream<ap_uint<8>> & Input_1,
        hls::stream<ap_uint<64>> & Output_1
        )
{
#pragma HLS INTERFACE axis register port=Input_1
#pragma HLS INTERFACE axis register port=Output_1

    static hls::stream<ap_uint<72>> out_str_dwc_5("out_str_dwc_5");
    static hls::stream<ap_uint<8>> out_mva_3("out_mva_3");
    static hls::stream<ap_uint<64>> out_str_dwc_6("out_str_dwc_6");

#pragma HLS dataflow
    str_dwc_5(Input_1, out_str_dwc_5);
    mva_3(out_str_dwc_5, out_mva_3);
    str_dwc_6(out_mva_3, out_str_dwc_6);
    str_mp_batch_1(out_str_dwc_6, Output_1);

}
